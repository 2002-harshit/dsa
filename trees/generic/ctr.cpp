...
public:
    ...
    void pre_pro_traverse(Node *root)
    {
        ...
        for (int i = 0; i < root->children.size(); i++)
        {
            cout << "Edge pre " << root->data << "--" << root->children[i]->data << endl;
            pre_pro_traverse(root->children[i]);
            cout << "Edge post " << root->data << "--" << root->children[i]->data << endl;
        }
    ...
    void mirror(Node *root)
    {
        ...
        //* bc fuddu sawaal tha, again try to remember the expectation, faith approach, faith mere bachhon ko mirror karna aata hai, bas phir apne bachhon ko reverse maaro, done!!
    }

    void level_order_line_wise_single_queue(Node *root)
    {
        //* again this apporach is also fairly basic, instead of 2 queues , it uses a single queue with separator as nullptr to separate between different levels;
        deque<Node *> list;
        list.push_back(root);
    ...
    Node *linearize_efficient(Node *root)
    {
        ...
        //* now our faith is when we call this function, it will linearize and also return the tail, so that we cann connect;

        Node *lastTail = linearize_efficient(root->children[root->children.size() - 1]); //* you dont need its tail, but you do need lineraization

        while (root->children.size() > 1)
        {
            Node *last = root->children[root->children.size() - 1];                                //* get the last child
            root->children.erase(root->children.begin() + root->children.size() - 1);              //* remove it from children
            Node *secondLastTail = linearize_efficient(root->children[root->children.size() - 1]); //* get second last childs tail
    ...
    bool find(Node *root, int val)
    {
        ...
        //* it is worth noticing in this question that the whole tree is not travelled through, you may notice that once the element is found, you return immediately and that answer propogates to the root.
...
int main()
{
    ...
    vector<int> vec = {10, 20, 50, -1, 60, -1, -1, 30, 70, -1, 80, 110, -1, 120, -1, -1, 90, -1, -1, 40, 100, -1, -1, 50, -1, -1};
    // vector<int> vec = {10, 20, 50, -1, 60, -1, -1, 30, 70, -1, 80, 1, 10, -1, 120, -1, -1, 90, -1, -1, 40, 100, -1, -1, -1};
    GenericTree gt;
    gt.create(vec);
